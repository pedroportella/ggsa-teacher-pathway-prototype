...
export function Card({ title, children, className, href, meta, tag, shadow = true }: CardProps) {
  return <article className={classNames('health-card', shadow && 'health-card--shadow', href && 'health-card--clickable', className)}><div className="health-card__body"><h2 className="health-card__title">{href ? <a className="health-card__link" href={href}>{title}</a> : title}</h2>{children && <div className="health-card__text">{children}</div>}{meta && <div className="health-card__date">{meta}</div>}{tag && <div className="health-card__tag">{tag}</div>}</div></article>;
}
...
export function Panel({ id, title, eyebrow, children, className, wide = false }: PanelProps) {
  return <section id={id} className={classNames('health-card health-card--shadow panel', wide && 'panel--wide', className)}><div className="health-card__body">{eyebrow && <p className="eyebrow">{eyebrow}</p>}<h2 className="health-card__title">{title}</h2>{children}</div></section>;
}
