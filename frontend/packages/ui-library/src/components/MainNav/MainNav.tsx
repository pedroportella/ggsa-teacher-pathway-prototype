        ...
        <div className="au-main-nav__dropdown">
          <ul className="au-link-list">
            {link.children.map((child) => (
              <li key={child.href}>
                <a aria-current={child.current ? 'page' : undefined} href={child.href}>
                  {child.label}
                </a>
              </li>
            ))}
...
export function MainNav({
  ...
  links,
  mobileLinks = [],
  withSearchToggle = true,
}: MainNavProps) {
  return (
    <nav
      aria-label="main"
      className={['au-main-nav', dark ? 'au-main-nav--dark' : undefined].filter(Boolean).join(' ')}
      id="main-navigation"
    >
        ...
        <div className="row">
          <div className="col-md-12">
            <div className="au-main-nav__content" id={id}>
              ...
              {withSearchToggle && (
                <button
                  className="au-main-nav__toggle au-main-nav__toggle--search au-main-nav__toggle--open"
                  type="button"
                >
                  <span
                    className="svg-inline--fa fa-fw au-main-nav__toggle--search__closed"
                    aria-hidden="true"
                  >
                    <svg viewBox="0 0 24 24">
                      <path d="M10 4a6 6 0 1 0 0 12 6 6 0 0 0 0-12zm8.7 16.1-4.2-4.2"></path>
                    </svg>
                  </span>
                  <span
                    className="svg-inline--fa fa-fw au-main-nav__toggle--search__open"
                    aria-hidden="true"
                  >
                    <svg viewBox="0 0 24 24">
